...
class _MoveRecord:
  def __init__(self, move, moved_piece, captured_piece, captured_square, prev_white_kingside, prev_white_queenside, prev_black_kingside, prev_black_queenside, prev_en_passant_square, prev_halfmove_clock, prev_fullmove_number):
...
class GameState:
  ...
  def copy(self):
    ...
    return GameState(new_board, self.side_to_move, self.white_kingside, self.white_queenside, self.black_kingside, self.black_queenside, self.en_passant_square, self.halfmove_clock, self.fullmove_number)
  ...
  def make_move(self, move):
    ...
    self.move_history.append(_MoveRecord(move, piece, captured_piece, captured_square, self.white_kingside, self.white_queenside, self.black_kingside, self.black_queenside, self.en_passant_square, self.halfmove_clock, self.fullmove_number))
    ...
    if move.is_castle():
      ...
      if move.flag == Move.CASTLE_KINGSIDE:
        ...
        board.state[row][0], board.state[row][3] = 0, 4 * side
